...
class SmartScorer:
  ...
  def _smart_1(
    ...
    import inspect
    # print(inspect.getmembers(score_matrix[0][0]))
    # print()
    # print(type(score_matrix[0][0]))
    
    # MORE FIXES ARRGGG
    # The other fix worked well, but the test is still failing. not sure what happened.
    if not isinstance(score_matrix[0][0], float):
      get_chf2 = np.vectorize(lambda x: x.score)
      score_matrix = get_chf2(score_matrix)
  ...
  def smart_score_precomputed(
    ...
    # EVEN MORE FIXES ARRGGG
    # Third time I need to add this, not sure where the root of the problem is but it seems like it's working
    if not isinstance(ref_score_matrix[0][0], float):
      get_chf2 = np.vectorize(lambda x: x.score)
      ref_score_matrix = get_chf2(ref_score_matrix)
